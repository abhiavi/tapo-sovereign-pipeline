...
from .media_stream.session import HttpMediaSession, KlapMediaSession
...
class Tapo:
    ...
    def __init__(
        ...
        # Store transport reference for KLAP media session use
        self._transport = self.transport if self.isKLAP else None
    ...
    def getMediaSession(self, stream_type: StreamType = None, start_time=""):
        ...
        # Use KLAP media session for KLAP-enabled firmware (1.5.0+)
        if self.isKLAP and self._transport is not None:
            self.logger.debugLog(
                f"Using KlapMediaSession for KLAP-enabled camera (firmware 1.5.0+)"
            )
            ...
            # Use legacy HTTP Digest Auth for older firmware
            self.logger.debugLog(
                f"Using HttpMediaSession for legacy camera (firmware <1.5.0)"
            )
